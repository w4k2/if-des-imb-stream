...
scores = np.load("scores_metrics.npy")
...
for j, clf in enumerate(clfs):
    ...
    for i, metric in enumerate(metrics):
        ...
        for d, drift in enumerate(drifts):
            ...
            mpl = np.zeros((len(seeds)*len(label_noises), len(methods)))
            rl = np.zeros((len(seeds)*len(label_noises), len(methods)))
            ...
            whole = np.concatenate((mean_mpl.reshape(1,len(methods)), mean_rl.reshape(1,len(methods))), axis=0)
            # print(mean_mpl)
