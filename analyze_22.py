...
def plot_runs(
    ...
    plt.tight_layout()
    plt.savefig("plots/experiment22/runs/%s/22_%s_%s_%s.eps" % (what, clfs[j], metrics[i], dependency[k]), bbox_inches='tight', dpi=250, pad_inches=0.0)
    plt.close()
...
def plot_radars(
    ...
    plt.savefig("plots/experiment22/radars/%s/22_%s_%s.eps" % (what, classifier_name, parameter_name), bbox_inches='tight', dpi=250, pad_inches=0.0)
    plt.close()
