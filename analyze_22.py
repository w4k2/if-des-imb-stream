...
def plot_runs(
    ...
    plt.tight_layout()
    plt.savefig("plots/experiment22/runs/%s/%s_%s_%s" % (what, clfs[j], metrics[i], dependency[k]), bbox_inches='tight', dpi=250)
    plt.close()
...
def plot_radars(
    ...
    plt.savefig("plots/experiment22/radars/%s/%s_%s.png" % (what, classifier_name, parameter_name), bbox_inches='tight', dpi=250)
    plt.close()
